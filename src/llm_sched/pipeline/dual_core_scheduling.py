"""Run-root workflow for SPEC-11 dual-core scheduling."""
...
from llm_sched.planning import plan_dual_core_schedule


class DualCoreSchedulingResult(BaseModel):
...
def run_dual_core_scheduling(run_root: str | Path) -> DualCoreSchedulingResult:
    ...
    try:
        ...
        schedule_ir = plan_dual_core_schedule(
            bound_nig_ir,
            memory_plan,
            tiling_plan,
            target_profile,
        ...
        schedule_ir_output_path = layout.artifacts_dir / "dual_core_schedule_ir.json"
        dump_ir_document(schedule_ir, schedule_ir_output_path)

        artifact_index["dual_core_schedule_ir"] = _relative_to_run(layout.run_root, schedule_ir_output_path)
        ...
        return DualCoreSchedulingResult(
            status="completed",
