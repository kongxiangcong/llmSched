"""Run-root workflow for static diagnosis workbench packaging."""
...
class DiagnosisWorkbenchResult(BaseModel):
...
def run_diagnosis_workbench(run_root: str | Path) -> DiagnosisWorkbenchResult:
    ...
    try:
        ...
        bundle_path = layout.run_root / Path(
            artifact_index.get("diagnosis_bundle", "reports/diagnosis_bundle.json")
        )
        if not bundle_path.is_file():
            raise FileNotFoundError(f"diagnosis_bundle not found at {bundle_path}")

        bundle = DiagnosisBundle.model_validate_json(bundle_path.read_text(encoding="utf-8"))
        ...
        workbench_manifest_path = layout.run_root / "diagnosis_workbench" / "workbench_manifest.json"
        artifact_index.update(
            {
                "diagnosis_workbench_entry": _relative_to_run(layout.run_root, entry_html_path),
                "diagnosis_workbench_manifest": _relative_to_run(
        ...
        return DiagnosisWorkbenchResult(
            status="completed",
        ...
        return DiagnosisWorkbenchResult(status="failed", diagnostics=diagnostics)
