"""Run-root workflow for SPEC-15 decode top-level evaluation."""
...
class DecodeEvaluationResult(BaseModel):
...
def run_decode_evaluation(run_root: str | Path) -> DecodeEvaluationResult:
    ...
    try:
        ...
        decode_report = build_decode_evaluation_report(
            manifest.run_id,
            scenario_profile,
            perf_summary_report,
            isa_coverage_report,
        ...
        report_output_path = layout.reports_dir / "decode_evaluation_report.json"
        _write_json_report(report_output_path, decode_report)

        artifact_index["decode_evaluation_report"] = _relative_to_run(layout.run_root, report_output_path)
        ...
        return DecodeEvaluationResult(status="failed", diagnostics=diagnostics)
...
def _write_json_report(path: Path, report: DecodeEvaluationReport) -> None:
    path.parent.mkdir(parents=True, exist_ok=True)
