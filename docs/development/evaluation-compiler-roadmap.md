...
| M3 能做架构评估 | SPEC-13,14,15,16 | `in_progress` | `SPEC-13` 已有正式 perf foundation，并新增 stable 的 per-node / per-layer perf summary、token-normalized phase attribution、phase occupied slots、phase-aware address-space pressure、phase-aware backing-store pressure、phase-aware memory-class pressure，以及 summary-grade `bandwidth_pressure_summary` / `vmem_pressure_summary`；`SPEC-14` 和 `SPEC-15` 已有 top-level foundation，且现已暴露 `node_hotspots` 与 `layer_breakdown`，并通过 `SPEC-16` 获得 formal top-level compare consumer、pressure-summary compare consumer 与 fitted hotspot/layer compare consumer；`SPEC-16` 已有 sweep/delta foundation、mode-aware compare summaries、phase-cycle/phase-share/phase-byte/phase-byte-share/phase-density/phase-cycle-components/phase-address-space-pressure/phase-backing-store-pressure/phase-memory-class-pressure/phase-schedule-compression/phase-occupied-slots/selected-phase-balance richer compare rows、pressure-grade `bandwidth_pressure_compare` / `vmem_pressure_compare`、grouped `metric_delta_groups`、fitted `node_deltas` / `fitted_layer_deltas`，以及 standalone `PhaseDCompareReport` artifact，同时 raw visualization packaging 也已能消费 grouped metric compare。Phase D 仍缺更深的 estimator、更强的 layer-level diff mode，以及更丰富的 compare modes。 |
...
- `SPEC-16`
  - richer diff mode
  - richer layer-level diff mode，而不只是 grouped summary metric compare
  - broader compare modes beyond current grouped scalar-plus-layer-plus-pressure surface
...
## 2026-03-13 SPEC-13 Phase Address-Space Pressure Checkpoint

- plan doc: `../plans/2026-03-13-spec-13-phase-address-space-pressure.md`
- `SPEC-13` phase attribution now carries per-phase `read/write bytes by address space`, so downstream consumers can inspect phase-aware DDR versus VMEM pressure directly from the canonical perf surface instead of reopening raw descriptor-analysis records.
- this slice stays on the existing `DescriptorIR/AnalysisIR -> PerfSummaryReport.phase_attribution -> Prefill/DecodeEvaluationReport` chain and reuses the current stage/role/address-space mapping logic rather than creating a second bandwidth model.
- new closure evidence:
  - `PerfPhaseSummary` now carries `read_bytes_by_address_space` and `write_bytes_by_address_space` with compatibility-friendly empty-dict defaults
  - performance estimation now aggregates per-phase address-space pressure by reusing the existing read/write role mapping and inferred DDR weight-byte logic already used for total-level bandwidth breakdown
  - `PrefillThroughputSummary` and `DecodeLatencySummary` preserve the richer structured phase surface automatically through their existing canonical `phase_attribution` handoff
...
- what this closes:
  - one `SPEC-13` bandwidth / VMEM breakdown gap where phase attribution still collapsed address-space pressure to phase-level total bytes only
  - one `SPEC-13 -> SPEC-14/15` handoff gap where top-level reports preserved phase cycles and bytes but not the underlying phase-aware memory-pressure mix
  - one future-consumer gap where later compare or visualization surfaces would otherwise have needed to reopen raw records just to answer phase-local DDR versus VMEM questions
- what still remains for `M3`:
  - deeper `SPEC-13` cycle fitting below the current summary-grade phase, occupied-slot, address-space-pressure, and critical-path surfaces
  - broader `SPEC-16` compare surfaces beyond the current scalar-plus-phase-plus-share-plus-byte-plus-byte-share-plus-phase-density-plus-highlight-plus-layer contract
  - any later `SPEC-19` compare expansion should stay downstream of the current report, sweep, and Phase D compare contracts

## 2026-03-13 SPEC-13 Phase Backing-Store Pressure Checkpoint

- plan doc: `../plans/2026-03-13-spec-13-phase-backing-store-pressure.md`
- `SPEC-13` phase attribution now carries per-phase `read/write bytes by backing_store`, so downstream consumers can inspect phase-local `vmem-local` versus staged/persistent DDR pressure directly from the canonical perf surface instead of reopening raw descriptor or memory-plan provenance.
- this slice stays on the existing `DescriptorIR/AnalysisIR -> PerfSummaryReport.phase_attribution -> Prefill/DecodeEvaluationReport` chain and reuses the current stage/role mapping plus `AddressField.backing_store` provenance rather than creating a second storage model.
- new closure evidence:
  - `PerfPhaseSummary` now carries `read_bytes_by_backing_store` and `write_bytes_by_backing_store` with compatibility-friendly empty-dict defaults
  - performance estimation now aggregates per-phase backing-store pressure by reusing the existing read/write role mapping, descriptor address provenance, and inferred staged DDR weight-byte logic already used for current summary modeling
  - legacy descriptor fields that expose `address_space` but omit `backing_store` now fall back to `vmem-local` for VMEM and staged/persistent DDR defaults where needed, keeping older fixtures and artifacts readable
  - `PrefillThroughputSummary` and `DecodeLatencySummary` preserve the richer structured phase surface automatically through their existing canonical `phase_attribution` handoff
  - focused contract/builder/workflow verification remains green (`22 passed`), and performance/prefill/decode CLI smoke now asserts the new JSON fields end to end (`6 passed`)
- what this closes:
  - one `SPEC-13` bandwidth / VMEM breakdown gap where phase attribution still knew address space but not whether DDR pressure was staged weights, persistent KV, or VMEM-local traffic
  - one `SPEC-13 -> SPEC-14/15` handoff gap where top-level reports preserved phase cycles, bytes, address-space mix, but not the underlying phase-aware storage provenance
  - one future-consumer gap where later compare or visualization surfaces would otherwise have needed to reopen raw descriptors just to answer phase-local backing-store questions
- what still remains for `M3`:
  - deeper `SPEC-13` cycle fitting below the current summary-grade phase, occupied-slot, address-space-pressure, backing-store-pressure, and critical-path surfaces
  - broader `SPEC-16` compare surfaces beyond the current scalar-plus-phase-plus-share-plus-byte-plus-byte-share-plus-phase-density-plus-highlight-plus-layer contract
  - any later `SPEC-19` compare expansion should stay downstream of the current report, sweep, and Phase D compare contracts

## 2026-03-13 SPEC-13 Phase Memory-Class Pressure Checkpoint

- plan doc: `../plans/2026-03-13-spec-13-phase-memory-class-pressure.md`
- `SPEC-13` phase attribution now carries per-phase `read/write bytes by memory_class`, so downstream consumers can inspect whether each phase is dominated by activation, weight, KV-cache, or quant-param traffic directly from the canonical perf surface instead of reopening raw storage bindings.
- this slice stays on the existing `DescriptorIR/AnalysisIR -> PerfSummaryReport.phase_attribution -> Prefill/DecodeEvaluationReport` chain and prefers direct reuse of `storage_binding_id -> memory_plan.storage_bindings[*].memory_class`, only falling back to minimal role-based classification when descriptor fields lack binding provenance.
- new closure evidence:
  - `PerfPhaseSummary` now carries `read_bytes_by_memory_class` and `write_bytes_by_memory_class` with compatibility-friendly empty-dict defaults
  - performance estimation now aggregates per-phase memory-class pressure by reusing storage-binding provenance for DDR-backed fields and preserving inferred compute weight reads as `WEIGHT`
  - local or legacy descriptor fields that lack `storage_binding_id` now fall back to activation/weight/quant/KV role classes, keeping older fixtures and VMEM-local surfaces readable without introducing a second planner model
  - `PrefillThroughputSummary` and `DecodeLatencySummary` preserve the richer structured phase surface automatically through their existing canonical `phase_attribution` handoff
  - focused contract/builder/workflow verification remains green (`22 passed`), and performance/prefill/decode CLI smoke now asserts the new JSON fields end to end (`6 passed`)
- what this closes:
  - one `SPEC-13` memory-pressure gap where phase attribution still knew backing store and address space but not which tensor class was actually driving that traffic
  - one `SPEC-13 -> SPEC-14/15` handoff gap where top-level reports preserved phase cycles, bytes, backing-store mix, but not the phase-local source-class view
  - one future-consumer gap where later compare or visualization surfaces would otherwise have needed to reopen raw storage bindings just to answer phase-local activation/weight/KV pressure questions
- what still remains for `M3`:
...
## 2026-03-14 SPEC-16 Phase Memory-Class Compare Checkpoint

- plan doc: `../plans/2026-03-14-spec-16-phase-memory-class-compare.md`
- `SPEC-16` compare now preserves fixed phase-local memory-class pressure rows, so downstream compare consumers can distinguish activation, weight, and KV-cache traffic per phase without reopening canonical perf reports or storage-binding provenance.
- this slice stays on the existing `PerfSummaryReport.phase_attribution -> Prefill/DecodeEvaluationReport -> SweepRunRecord.metrics -> Sweep*CompareSummary -> PhaseDCompareReport -> Visualization compare summary` chain and mirrors the current address-space/backing-store compare pattern instead of introducing dynamic compare payloads.
- new closure evidence:
  - `SweepRunRecord.metrics` now exports fixed per-phase memory-class rows for `ACTIVATION`, `WEIGHT`, and `KV_CACHE` across read and write directions
  - `SweepPrefillCompareSummary` / `SweepDecodeCompareSummary` now carry the same fixed memory-class scalar deltas with compatibility-friendly zero defaults
  - `PhaseDCompareReport` and visualization compare summaries now forward the same memory-class rows directly, keeping compare serialization stable across standalone report and workbench-facing consumers
  - focused compare-chain verification remains green (`14 passed`)
- what this closes:
  - one `SPEC-16` compare gap where canonical phase-local memory-class pressure from `SPEC-13` still disappeared once results crossed into sweep compare artifacts
  - one standalone-report gap where `PhaseDCompareReport` could surface phase cycles, bytes, backing-store pressure, address-space pressure, schedule compression, occupied slots, and balance, but not which tensor class actually dominated that traffic
  - one visualization gap where compare summaries could not yet expose activation/weight/KV-cache provenance even though canonical perf reports already carried it
- what still remains for `M3`:
...
- plan doc: `../plans/2026-03-20-spec-13-pressure-summary.md`
- `SPEC-13` now exposes summary-grade `bandwidth_pressure_summary` and `vmem_pressure_summary` alongside the existing raw breakdown maps.
- this slice stays intentionally narrow:
  - no new estimator math
  - no `SPEC-14/15` report-builder adoption yet
  - no `SPEC-16` compare-contract changes yet
- new closure evidence:
  - focused verification is green: `python -m pytest tests/unit/contracts/test_perf_report.py tests/unit/analysis/test_perf_summary_builder.py tests/unit/pipeline/test_performance_estimation_workflow.py tests/smoke/test_cli_run_performance_estimation.py -q` -> `12 passed`
- what this closes:
  - one readability gap where downstream consumers had to reconstruct peak bandwidth and hottest-VMEM-region signals from multiple raw maps
  - one contract gap where `SPEC-14/15/19` had no compact summary-grade pressure surface to reuse directly
- what still remains for `M3`:
  - `SPEC-14/15` still need to adopt the richer `SPEC-13` pressure surface where it improves report readability
  - `SPEC-16` still needs richer compare-grade exposure for pressure summaries
  - deeper cycle fitting remains future `SPEC-13` work, separate from this summary slice

## 2026-03-20 SPEC-14/15 Pressure Summary Adoption Checkpoint

- plan doc: `../plans/2026-03-20-spec-14-15-pressure-summary-adoption.md`
- `PrefillEvaluationReport` and `DecodeEvaluationReport` now expose `bandwidth_pressure_summary` and `vmem_pressure_summary` directly.
- this slice stays strictly downstream of `SPEC-13`:
  - no new estimator math
  - no report-local reconstruction of pressure signals
  - no `SPEC-16` compare contract changes yet
- new closure evidence:
  - focused verification is green: `python -m pytest tests/unit/analysis/test_prefill_report_builder.py tests/unit/analysis/test_decode_report_builder.py tests/unit/pipeline/test_prefill_evaluation_workflow.py tests/unit/pipeline/test_decode_evaluation_workflow.py tests/smoke/test_phase_d_prefill_foundation_matrix.py tests/smoke/test_phase_d_decode_foundation_matrix.py -q` -> `16 passed`
- what this closes:
  - one adoption gap where `SPEC-13` already had reusable summary-grade pressure signals but `SPEC-14/15` still only exposed raw hotspot/memory maps
  - one downstream readability gap where prefill/decode consumers could not access the same peak-pressure surface as the perf summary
- what still remains for `M3`:
  - `SPEC-16` still needs compare-grade pressure-summary exposure
  - `SPEC-14/15` still need richer top-level compare and deeper scenario-specific analysis beyond this adoption slice
...
## 2026-03-14 SPEC-16 Workspace Summary Stack Checkpoint

- plan doc: `../plans/2026-03-14-spec-16-workspace-summary-stack.md`
- catalog compare workspace summary cells now use a shared `summary-stack` layout wrapper, so semantic tags and their associated delta, ratio, shared-summary, and sweep-summary content all follow the same vertical spacing and read more consistently row to row.
- this slice stays entirely inside the catalog static builder and CSS, and changes only presentation structure plus styles without changing compare semantics, payloads, filters, or links.
- new closure evidence:
  - workspace compare table cells for `Primary Delta`, `Primary Ratio`, `Shared Metric Deltas`, and `Sweep Layer Deltas` now render a common `summary-stack` / `summary-stack-value` wrapper structure
  - catalog styles now define shared stack spacing for those wrappers instead of relying on ad hoc tag-plus-text flow in each cell
  - focused visualization verification remains green (`18 passed`) across catalog/workbench builder tests plus visualization packaging workflow coverage
- what this closes:
  - one workspace readability gap where summary tags across adjacent columns had inconsistent spacing and alignment even after semantic tagging landed
  - one UI maintenance gap where each compare summary cell encoded its own local tag/content rhythm instead of reusing a shared layout primitive
  - one generated-asset testing gap where the normalized workspace summary structure was not visible directly in emitted assets
- what still remains for `M3`:
  - broader `SPEC-16` compare surfaces beyond the current scalar-plus-phase-plus-share-plus-byte-plus-byte-share-plus-backing-store-plus-memory-class-plus-address-space-plus-cycle-components-plus-schedule-compression-plus-occupied-slot-plus-balance-plus-highlight-plus-layer contract
  - deeper `SPEC-13` cycle fitting below the current summary-grade phase and critical-path surfaces
  - any later `SPEC-19` compare expansion should stay downstream of the current report, sweep, and Phase D compare contracts

## 2026-03-14 SPEC-16 Workspace Summary Stack Helper Checkpoint

- plan doc: `../plans/2026-03-14-spec-16-workspace-summary-stack-helper.md`
- workspace compare table rendering now routes all summary-stack cell markup through a shared `renderWorkspaceSummaryStack` helper, reducing repeated HTML fragments while keeping the generated delta, ratio, shared-summary, and sweep-summary cell structure unchanged.
- this slice is a pure builder refactor inside the catalog static JS template and does not alter compare semantics, CSS behavior, payloads, or links.
- new closure evidence:
  - catalog `renderWorkspaceSummaryStack` now owns the shared `summary-stack` / `summary-stack-value` wrapper markup
  - `buildWorkspaceCompareRows` now reuses that helper across all four summary columns instead of inlining duplicated wrapper HTML
  - focused visualization verification remains green (`18 passed`) across catalog/workbench builder tests plus visualization packaging workflow coverage
- what this closes:
  - one maintenance gap where workspace compare summary cells repeated the same wrapper fragment four times, making future layout edits easy to miss in one column
  - one drift risk where subsequent compare-cell changes could have forked the summary-stack structure even though the intended UI primitive was already shared
  - one generated-asset testing gap where the normalized helper itself was not visible directly in emitted `catalog/assets/app.js`
- what still remains for `M3`:
  - broader `SPEC-16` compare surfaces beyond the current scalar-plus-phase-plus-share-plus-byte-plus-byte-share-plus-backing-store-plus-memory-class-plus-address-space-plus-cycle-components-plus-schedule-compression-plus-occupied-slot-plus-balance-plus-highlight-plus-layer contract
  - deeper `SPEC-13` cycle fitting below the current summary-grade phase and critical-path surfaces
  - any later `SPEC-19` compare expansion should stay downstream of the current report, sweep, and Phase D compare contracts
...
## 2026-03-14 SPEC-16 Workspace Direction Tag Titles Checkpoint

- plan doc: `../plans/2026-03-14-spec-16-workspace-direction-tag-titles.md`
- catalog workspace compare tags now share a dedicated `buildTitledDirectionTagMarkup(title, semanticClass, label)` helper, so summary, ratio, and sweep title attributes no longer depend on brittle string replacement or repeated inline span templates.
- this slice stays entirely inside the catalog static JS builder and is a pure markup-hardening refactor: it preserves current semantic classes, labels, title strings, payload contracts, and workspace compare layout.
- new closure evidence:
  - catalog now exposes `buildTitledDirectionTagMarkup` beside the existing direction-tag helpers
  - `buildWorkspaceCompareSummaryTag`, `buildWorkspaceCompareRatioSummaryTag`, and `buildWorkspaceSweepSummaryTag` now call the titled helper directly with `workspace summary`, `workspace ratio summary`, and `workspace sweep summary`
  - focused visualization verification remains green (`18 passed`) across catalog/workbench builder tests plus visualization packaging workflow coverage
- what this closes:
  - one maintenance gap where workspace summary and ratio tags depended on `.replace(...)` against another helper's emitted markup
  - one drift risk where future direction-tag template changes could have silently stripped or misplaced workspace-specific title attributes
  - one generated-asset testing gap where titled workspace direction-tag construction was not visible directly in emitted `catalog/assets/app.js`
- what still remains for `M3`:
  - broader `SPEC-16` compare surfaces beyond the current scalar-plus-phase-plus-share-plus-byte-plus-byte-share-plus-backing-store-plus-memory-class-plus-address-space-plus-cycle-components-plus-schedule-compression-plus-occupied-slot-plus-balance-plus-highlight-plus-layer contract
  - deeper `SPEC-13` cycle fitting below the current summary-grade phase and critical-path surfaces
  - any later `SPEC-19` compare expansion should stay downstream of the current report, sweep, and Phase D compare contracts
...
## 2026-03-14 SPEC-16 Workspace Sweep State Helper Checkpoint

- plan doc: `../plans/2026-03-14-spec-16-workspace-sweep-state-helper.md`
- catalog workspace sweep tags now separate sweep-state detection into `resolveWorkspaceSweepSummaryState(sweepComparison)`, so the titled tag builder no longer owns both state classification and markup emission in one branchy helper.
- this slice stays entirely inside the catalog static JS builder and is a pure refactor: it preserves the current `workspace sweep summary` title, `none` / `mixed` / `candidate regressions` labels, semantic classes, and workspace compare rendering.
- new closure evidence:
  - catalog now exposes `resolveWorkspaceSweepSummaryState` beside the existing workspace summary helpers
  - `buildWorkspaceSweepSummaryTag` now delegates state classification to that helper and only maps the result through `buildTitledDirectionTagMarkup`
  - focused visualization verification remains green (`18 passed`) across catalog/workbench builder tests plus visualization packaging workflow coverage
- what this closes:
  - one cohesion gap where workspace sweep summary logic mixed layer-delta classification and titled tag markup in the same helper
  - one drift risk where future sweep-state additions could have required touching markup flow instead of extending a dedicated state resolver
  - one generated-asset testing gap where workspace sweep-state classification was not visible directly in emitted `catalog/assets/app.js`
- what still remains for `M3`:
  - broader `SPEC-16` compare surfaces beyond the current scalar-plus-phase-plus-share-plus-byte-plus-byte-share-plus-backing-store-plus-memory-class-plus-address-space-plus-cycle-components-plus-schedule-compression-plus-occupied-slot-plus-balance-plus-highlight-plus-layer contract
  - deeper `SPEC-13` cycle fitting below the current summary-grade phase and critical-path surfaces
  - any later `SPEC-19` compare expansion should stay downstream of the current report, sweep, and Phase D compare contracts

## 2026-03-14 SPEC-16 Workspace Primary Scalar Helper Checkpoint

- plan doc: `../plans/2026-03-14-spec-16-workspace-primary-scalar-helper.md`
- catalog workspace summary and ratio tags now share `resolveWorkspacePrimaryScalarDelta(baselineEntry, candidateEntry)`, so both columns consume the same primary-metric scalar-delta payload instead of rebuilding it separately before passing into the titled scalar helper.
- this slice stays entirely inside the catalog static JS builder and is a pure helper refactor: it preserves current titles, semantic classes, labels, ratio behavior, and workspace compare rendering.
- new closure evidence:
  - catalog now exposes `resolveWorkspacePrimaryScalarDelta` alongside the titled scalar and sweep-state helpers
  - `buildWorkspaceCompareSummaryTag` and `buildWorkspaceCompareRatioSummaryTag` now both delegate baseline/candidate primary-metric extraction to that resolver
  - focused visualization verification remains green (`18 passed`) across catalog/workbench builder tests plus visualization packaging workflow coverage
- what this closes:
  - one duplication gap where workspace summary and ratio tags still rebuilt the same primary scalar-delta object after semantic markup had already been shared
  - one drift risk where future primary-metric extraction changes could have updated one workspace column but missed the other
  - one generated-asset testing gap where the shared primary scalar resolver was not visible directly in emitted `catalog/assets/app.js`
- what still remains for `M3`:
  - broader `SPEC-16` compare surfaces beyond the current scalar-plus-phase-plus-share-plus-byte-plus-byte-share-plus-backing-store-plus-memory-class-plus-address-space-plus-cycle-components-plus-schedule-compression-plus-occupied-slot-plus-balance-plus-highlight-plus-layer contract
  - deeper `SPEC-13` cycle fitting below the current summary-grade phase and critical-path surfaces
  - any later `SPEC-19` compare expansion should stay downstream of the current report, sweep, and Phase D compare contracts
...
- what still remains for `M3`:
  - visualization-specific fitted compare payloads remained out of scope for this slice; fitted hotspot/layer compare landed in the follow-on `2026-03-15-spec-16-fitted-hotspot-layer-compare` slice
...
## 2026-03-15 SPEC-16 Fitted Hotspot/Layer Compare Checkpoint

- plan doc: `../plans/2026-03-15-spec-16-fitted-hotspot-layer-compare.md`
- `SPEC-16` sweep compare surfaces and the standalone `PhaseDCompareReport` now consume the richer fitted `node_hotspots` / `layer_breakdown` rows as compare-grade `node_deltas` and `fitted_layer_deltas`, while keeping the existing estimated `layer_deltas` semantics unchanged.
- this slice stays intentionally narrow: it preserves fitted hotspot/layer rows in sweep-local compare outputs and standalone compare reports without changing visualization consumers.
- new closure evidence:
  - `SweepComparison` now exposes `node_deltas` and `fitted_layer_deltas` in parallel with the existing estimated `layer_deltas`
  - `PhaseDCompareReport` prefill/decode rows now expose `node_delta_count`, `fitted_layer_delta_count`, `node_deltas`, and `fitted_layer_deltas`
  - focused Task 1 + Task 2 regression remains green (`40 passed`) across fitted report builders, workflows, sweep compare, standalone compare, and smoke coverage
- what this closes:
  - one compare-surface gap where fitted hotspot/layer rows were preserved in sweep run records but not yet consumed by compare outputs
  - one standalone-report gap where `run-phase-d-compare` could not yet emit fitted hotspot/layer compare rows already present in `sweep_delta_report.json`
  - one contract gap where compare/report consumers only had estimated `layer_deltas` and no parallel fitted hotspot/layer surfaces
- what still remains for `M3`:
  - visualization-specific fitted compare payload adoption remains later scope
  - broader `SPEC-16` compare expansion beyond the current scalar-plus-phase-plus-share-plus-byte-plus-byte-share-plus-backing-store-plus-memory-class-plus-address-space-plus-cycle-components-plus-schedule-compression-plus-occupied-slot-plus-balance-plus-hotspot-plus-layer contract remains later work
  - deeper cycle fitting remains in `SPEC-13`, not in these compare builders
...
- what still remains for `M3`:
  ...
  - richer compare-grade exposure of fitted cycles remains later `SPEC-16` work
  - any future compare or workbench expansion should continue consuming the existing report and compare contracts instead of reopening this batch's summary-grade surfaces
## 2026-03-20 SPEC-16 Fitted Layer Diff Visualization Update

- `SPEC-16 -> SPEC-18/19` visualization adoption has advanced one slice:
  - visualization bundle compare payloads now preserve `fitted_layer_deltas`
  - visualization catalog artifacts now preserve `fitted_layer_deltas`
  - catalog compare/workspace now supports fitted-focused layer diff modes:
    - `top-by-fitted-work`
    - `fitted-regressions-only`
  - workbench sweep rendering/export now includes fitted layer delta rows and focused fitted-layer snapshot summaries
- this moves the remaining `SPEC-16` gap from “fitted layer compare missing downstream” to:
  - richer compare modes beyond the current grouped scalar plus estimated/fitted layer plus pressure surface
  - deeper compare/workspace drill-down
  - any later block-level or execution-parallel diff work
...
- `SPEC-19` catalog workspace now has workspace-local `Copy Workspace Link`, `Export Workspace JSON`, and `Export Workspace SVG` controls.
- those exports now preserve focused workspace context in summary-grade metadata:
  ...
  - `Focused Sweep Layer`
- this closes one concrete `SPEC-19` workflow gap where the new workspace drilldown could be inspected locally but not copied or snapshotted from the catalog itself.
- focused verification for this slice:
  - `python -m pytest tests/unit/visualization/test_catalog_builder.py tests/unit/pipeline/test_visualization_catalog_workflow.py -q` -> `11 passed`
...
- next best follow-on:
  - keep `SPEC-16` as the mainline blocker and push richer compare modes there first
  - only then return to `SPEC-19` for higher-value interaction polish above the now-stable export path
