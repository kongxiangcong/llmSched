...
| Phase C | SPEC-08, SPEC-09, SPEC-10, SPEC-11, SPEC-12 | `in_progress` | `SPEC-08` foundation 已稳定，`SPEC-09` 已有 tile-planner foundation，`SPEC-10` single-core scheduler foundation 已稳定，`SPEC-11` dual-core scheduler foundation 已稳定，`SPEC-12` descriptor/ISA workflow 已稳定。 |
...
| SPEC-08 | VMEM/KV/地址规划器 | C | `in_progress` | 已有 `MemoryPlanArtifact`、静态 VMEM/KV planner、workflow/CLI foundation、四象限 smoke gate、KV address diagnostics，以及 fit-aware `M_tile` shrink、`EMBEDDING_LOOKUP` / `LAYOUT_FALLBACK` / `GEGLU` / `ROPE_TABLE` / `ROPE` working-set heuristics。 | 仍缺 lifetime reuse、更广覆盖的 capacity reasoning、真实 DDR binding 和进入 `SPEC-09` 的稳定输入面。 |
| SPEC-09 | Tile 候选规划器 | C | `in_progress` | 已有 `TilingPlanArtifact`、`plan_tiling_artifact(...)`、`run-tile-planning` workflow/CLI，以及 Gemma3 四象限 smoke gate。 | 仍缺更完整的 candidate search、更多 macro coverage、以及进入 `SPEC-10/11` 前的调度消费面收口。 |
| SPEC-10 | 单核映射与调度器 | C | `in_progress` | 已有扩展后的 `ScheduleIR`、`plan_single_core_schedule(...)`、`run-single-core-scheduling` workflow/CLI 和 single-core smoke gate。 | 仍缺更多 macro coverage、资源重叠搜索、以及进入 `SPEC-12` 前的 descriptor-facing 收口。 |
| SPEC-11 | 双核映射与调度器 | C | `in_progress` | 已有 dual-core `ScheduleIR` foundation、deterministic core assignment / transfer / barrier 语义、workflow 与 smoke gate。 | 仍缺更广覆盖、调度策略搜索以及进入 `SPEC-12` 前的 descriptor-facing 收口。 |
| SPEC-12 | 描述符生成与 ISA 覆盖映射 | C | `in_progress` | 已有扩展后的 `DescriptorIR` contract、`ISACoverageReport` contract、deterministic descriptor builder、run-root workflow、CLI/smoke gate 和 handoff；descriptor 现已具备 stage-complete field invariants、positive DMA lengths 和 stage-aware ISA gap taxonomy。 | 仍缺更完整的 opcode/field packing、最终地址字段编码和更高保真 descriptor encoding 收口。 |
...
| SPEC-14 | Prefill 评估流水线 | D | `in_progress` | 已有 `PrefillEvaluationReport` contract、prefill report builder、run-root workflow、CLI 和 `single-core/dual-core x prefill` smoke foundation。 | 仍缺 layer-level prefill 视图、单/双核对比输出和与 `SPEC-15/16` 的统一 top-level eval 收口。 |
| SPEC-15 | Decode 评估流水线 | D | `in_progress` | 已有 `DecodeEvaluationReport` contract、decode report builder、run-root workflow、CLI 和 `single-core/dual-core x decode` smoke foundation。 | 仍缺更细 token latency 拆解、`kv_len` sweep aggregation 和与 `SPEC-16/18` 的统一 top-level eval 收口。 |
...
| SPEC-18 | 可视化数据服务 | E | `in_progress` | 已有 `VisualizationBundle` contract、run-root packaging workflow、CLI 和 Gemma3 visualization smoke foundation。 | 仍缺 live query service、多运行 catalog、深层 drill-down 数据和更正式的 API/service 层。 |
| SPEC-19 | 可视化分析工作台 | E | `in_progress` | 已有 `VisualizationWorkbenchArtifact`、static workbench builder、`run-visualization-workbench` workflow/CLI、支持显式 `run_root` 与 `sweep_root/workspace_root` 发现的 cross-run catalog foundation，以及 graph/timeline 搜索过滤、timeline block drill-down、catalog search、scenario grouping/navigation、panel deep-link routing、workbench 内部的 descriptor/block/tensor cross-links，以及 saved-view link copy / active-panel JSON export / active-panel SVG export / catalog compare tray / baseline-pinned compare workspace / baseline-candidate role swap / same-scenario versus all-visible compare scope。 | 仍缺 richer screenshot workflow、更丰富的 workspace 导航和多指标 compare modes。 |
...
| M2 能映射到硬件 | SPEC-08,09,10,11,12 | `in_progress` | `SPEC-08` 与 `SPEC-09` foundation 已稳定，`SPEC-10` single-core、`SPEC-11` dual-core 和 `SPEC-12` descriptor workflow 已打通，但更高保真调度/descriptor 收口仍未完成。 |
...
- `SPEC-09`
  ...
  - 更广的 macro coverage
  - 为 scheduler 提供更稳定的 candidate ranking 输入
- `SPEC-10/11`
  - 扩大 scheduler macro coverage
  - 增强资源冲突与重叠建模
  - 把 single-core / dual-core schedule artifact 收成真正可供 descriptor 消费的稳定面
- `SPEC-12`
  - 更完整的 opcode / field packing
  - 最终地址字段编码
  - 更可信的 descriptor encoding closure
...
1. 优先继续 `Phase C / M2`
   - 首选方向：`SPEC-12` hardening
   - 原因：`SPEC-08/09/10/11/12` 都已有 foundation，但 `DescriptorIR -> ISA coverage` 仍是连接 schedule 层和评估层的关键收口面
