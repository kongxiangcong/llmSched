...
def test_plan_single_core_schedule_uses_latency_first_decode_candidate() -> None:
    ...
    block = next(
        ...
        if block.node_id == "nig.node.attn.decode" and block.stage == "compute"
    )
    assert block.tiling_candidate_id.endswith(".m1.n128.k128")


def test_plan_single_core_schedule_allows_mxu_compute_during_sdpa_decode_stream() -> None:
    from llm_sched.config.loader import load_scenario_profile, load_target_profile
    from llm_sched.planning.memory_planner import plan_memory_artifact
    from llm_sched.planning.single_core_scheduler import plan_single_core_schedule
    from llm_sched.planning.tile_planner import plan_tiling_artifact
    ...
    bound_nig = _make_bound_nig_ir(
        [
            _make_sdpa_decode_node(node_id="nig.node.attn.decode", shape=[1, 1, 1024]),
            _make_wdq_gemm_node(
    ...
    decode_compute = blocks["sched.nig.node.attn.decode.compute"]
    wdq_compute = blocks["sched.nig.node.linear.tail.compute"]

    assert decode_compute.resource_set == ["DMA", "VPU"]
    assert wdq_compute.resource_set == ["WDQ", "MXU"]
    assert wdq_compute.issue_slot >= decode_compute.issue_slot
    assert wdq_compute.issue_slot < decode_compute.issue_slot + decode_compute.duration_slots


def test_plan_single_core_schedule_allows_vpu_helper_during_sdpa_decode_dma_tail() -> None:
    ...
    blocks = {block.block_id: block for block in schedule.blocks}
    decode_compute = blocks["sched.nig.node.attn.decode.compute"]
    rope_table_dma = blocks["sched.nig.node.rope_table.dma_in"]
    helper_prepare = blocks["sched.nig.node.helper.prepare"]
    ...
    assert rope_table_dma.issue_slot + rope_table_dma.duration_slots <= vpu_release
...
def test_plan_single_core_schedule_prefers_candidate_rank_over_m_tile() -> None:
    from llm_sched.config.loader import load_scenario_profile, load_target_profile
...
def test_plan_single_core_schedule_covers_untiled_vpu_and_dma_macros() -> None:
    from llm_sched.config.loader import load_scenario_profile, load_target_profile
    ...
    bound_nig = _make_bound_nig_ir(
        [
            _make_passthrough_node("nig.node.norm", "RMSNORM", [1, 128, 1024]),
            _make_passthrough_node("nig.node.add", "ELEM_ADD", [1, 128, 1024]),
    ...
    assert [block.stage for block in schedule.blocks if block.node_id == "nig.node.norm"] == [
        "dma_in",
        "compute",
        "store",
    ]
    assert [block.resource_set for block in schedule.blocks if block.node_id == "nig.node.norm"] == [
        ["DMA"],
        ["VPU"],
        ["DMA"],
    ]
    assert [block.stage for block in schedule.blocks if block.node_id == "nig.node.add"] == [
        "dma_in",
        "compute",
        "store",
    ]
    assert [block.stage for block in schedule.blocks if block.node_id == "nig.node.kvload"] == ["dma_in"]
    assert [block.stage for block in schedule.blocks if block.node_id == "nig.node.kvstore"] == ["store"]
    assert all(block.tiling_candidate_id is None for block in schedule.blocks)


def test_plan_single_core_schedule_emits_overlap_issue_slots_for_dma_and_compute() -> None:
    from llm_sched.config.loader import load_scenario_profile, load_target_profile
    from llm_sched.planning.memory_planner import plan_memory_artifact
    from llm_sched.planning.single_core_scheduler import plan_single_core_schedule
    from llm_sched.planning.tile_planner import plan_tiling_artifact
    ...
    blocks = {block.block_id: block for block in schedule.blocks}
    assert blocks["sched.nig.node.linear.0.dma_in"].issue_slot == 0
    assert blocks["sched.nig.node.linear.0.compute"].depends_on == ["sched.nig.node.linear.0.dma_in"]
    assert blocks["sched.nig.node.linear.1.compute"].depends_on == [
        "sched.nig.node.linear.1.dma_in",
        "sched.nig.node.linear.0.store",
    ]
    assert blocks["sched.nig.node.linear.0.dma_in"].duration_slots > 1
    assert blocks["sched.nig.node.linear.0.compute"].duration_slots > 1
    assert blocks["sched.nig.node.linear.0.store"].duration_slots > 1
    assert blocks["sched.nig.node.linear.0.compute"].issue_slot >= (
        blocks["sched.nig.node.linear.0.dma_in"].issue_slot
        + blocks["sched.nig.node.linear.0.dma_in"].duration_slots
    )
    assert blocks["sched.nig.node.linear.1.compute"].issue_slot >= (
        blocks["sched.nig.node.linear.0.store"].issue_slot
        + blocks["sched.nig.node.linear.0.store"].duration_slots
    )
    assert blocks["sched.nig.node.linear.1.store"].issue_slot >= (
        blocks["sched.nig.node.linear.1.compute"].issue_slot
        + blocks["sched.nig.node.linear.1.compute"].duration_slots
    )


def test_plan_single_core_schedule_allows_mxu_reuse_during_sdpa_vpu_tail() -> None:
    from llm_sched.config.loader import load_scenario_profile, load_target_profile
    from llm_sched.planning.memory_planner import plan_memory_artifact
    from llm_sched.planning.single_core_scheduler import plan_single_core_schedule
    from llm_sched.planning.tile_planner import plan_tiling_artifact
    ...
    bound_nig = _make_bound_nig_ir(
        [
            _make_sdpa_node(node_id="nig.node.attn.prefill", shape=[1, 128, 1024]),
            _make_wdq_gemm_node(
                node_id="nig.node.linear.tail",
                output_shape=[1, 128, 1024],
                group_size=128,
    ...
    blocks = {block.block_id: block for block in schedule.blocks}
    sdpa_compute = blocks["sched.nig.node.attn.prefill.compute"]
    wdq_compute = blocks["sched.nig.node.linear.tail.compute"]
...
def test_plan_single_core_schedule_specializes_geglu_compute_resources() -> None:
    from llm_sched.config.loader import load_scenario_profile, load_target_profile
    from llm_sched.planning.memory_planner import plan_memory_artifact
    from llm_sched.planning.single_core_scheduler import plan_single_core_schedule
    from llm_sched.planning.tile_planner import plan_tiling_artifact
    ...
    bound_nig = _make_bound_nig_ir(
        [
            _make_passthrough_node("nig.node.geglu", "GEGLU", [1, 128, 1024]),
            _make_wdq_gemm_node(
                node_id="nig.node.linear.tail",
                output_shape=[1, 128, 1024],
                group_size=128,
    ...
    blocks = {block.block_id: block for block in schedule.blocks}
    geglu_compute = blocks["sched.nig.node.geglu.compute"]
    wdq_compute = blocks["sched.nig.node.linear.tail.compute"]
...
def test_plan_single_core_schedule_allows_vpu_issue_after_geglu_prefix() -> None:
...
def test_plan_single_core_schedule_keeps_vpu_helper_after_sdpa_vpu_tail() -> None:
    from llm_sched.arch.capabilities import ArchitectureCapabilities
    from llm_sched.config.loader import load_scenario_profile, load_target_profile
    from llm_sched.planning.memory_planner import plan_memory_artifact
    from llm_sched.planning.schedule_duration import estimate_stage_resource_reservations
    ...
    bound_nig = _make_bound_nig_ir(
        [
            _make_sdpa_node("nig.node.attn.prefill", [1, 128, 1024]),
    ...
    sdpa_compute = blocks["sched.nig.node.attn.prefill.compute"]
    helper_prepare = blocks["sched.nig.node.helper.prepare"]
    helper_compute = blocks["sched.nig.node.helper.compute"]
    sdpa_node = next(node for node in bound_nig.nodes if node.node_id == "nig.node.attn.prefill")
    sdpa_candidate = next(
        candidate
        for candidate in tiling_plan.candidates
...
def test_plan_single_core_schedule_allows_vpu_issue_after_sdpa_prefix() -> None:
    from llm_sched.arch.capabilities import ArchitectureCapabilities
    ...
    target = load_target_profile(repo_root / "profiles" / "targets" / "riscv_npu_single_core_v1.json")
    capabilities = ArchitectureCapabilities.from_target_profile(target)
    sdpa_node = _make_sdpa_node("nig.node.attn.prefill", [1, 128, 1024])
    candidate = _make_tile_candidate("fixture.sdpa.compute", "nig.node.attn.prefill", "SDPA", 48, 128, 128)
...
def test_plan_single_core_schedule_allows_gemm_before_wdq_mxu_prefix_ends() -> None:
    from llm_sched.config.loader import load_scenario_profile, load_target_profile
    from llm_sched.contracts.tiling_plan import TilingPlanArtifact
    from llm_sched.planning.memory_planner import plan_memory_artifact
    from llm_sched.planning.schedule_duration import estimate_stage_resource_reservations
    ...
    blocks = {block.block_id: block for block in schedule.blocks}
    wdq_compute = blocks["sched.nig.node.wdq.compute"]
    gemm_dma_in = blocks["sched.nig.node.gemm.dma_in"]
    gemm_compute = blocks["sched.nig.node.gemm.compute"]
...
def test_plan_single_core_schedule_starts_vpu_helper_at_rmsnorm_gemm_prefix_end() -> None:
    from llm_sched.arch.capabilities import ArchitectureCapabilities
    from llm_sched.config.loader import load_scenario_profile, load_target_profile
    from llm_sched.contracts.tiling_plan import TilingPlanArtifact
    from llm_sched.planning.memory_planner import plan_memory_artifact
    ...
    blocks = {block.block_id: block for block in schedule.blocks}
    rg_compute = blocks["sched.nig.node.rg.compute"]
    helper_prepare = blocks["sched.nig.node.helper.prepare"]
    helper_compute = blocks["sched.nig.node.helper.compute"]
...
def test_plan_single_core_schedule_allows_dma_overlap_during_wdq_dma_tail() -> None:
    from llm_sched.arch.capabilities import ArchitectureCapabilities
    from llm_sched.config.loader import load_scenario_profile, load_target_profile
    from llm_sched.planning.memory_planner import plan_memory_artifact
    from llm_sched.planning.schedule_duration import estimate_stage_resource_reservations
    ...
    blocks = {block.block_id: block for block in schedule.blocks}
    wdq_dma = blocks["sched.nig.node.wdq.dma_in"]
    kvload_dma = blocks["sched.nig.node.kvload.dma_in"]
...
def test_plan_single_core_schedule_allows_dma_before_kvstore_dma_window() -> None:
    from llm_sched.arch.capabilities import ArchitectureCapabilities
    from llm_sched.config.loader import load_scenario_profile, load_target_profile
    from llm_sched.contracts.tiling_plan import TilingPlanArtifact
    from llm_sched.planning.memory_planner import plan_memory_artifact
    ...
    blocks = {block.block_id: block for block in schedule.blocks}
    kvstore_store = blocks["sched.nig.node.kvstore.store"]
    elem_add_dma = blocks["sched.nig.node.elem.add.dma_in"]
    ...
    assert elem_add_dma.issue_slot == 0
    assert elem_add_dma.issue_slot < kvstore_store.issue_slot + dma_window[1]


def test_plan_single_core_schedule_allows_dma_at_rmsnorm_store_issue_with_vpu_prefix() -> None:
    from llm_sched.arch.capabilities import ArchitectureCapabilities
    from llm_sched.config.loader import load_target_profile
    from llm_sched.planning.schedule_duration import estimate_stage_resource_reservations
    from llm_sched.planning.schedule_reservations import (
...
def test_plan_single_core_schedule_allows_dma_at_elem_add_store_issue_with_vpu_prefix() -> None:
    from llm_sched.arch.capabilities import ArchitectureCapabilities
    from llm_sched.config.loader import load_target_profile
    from llm_sched.planning.schedule_duration import estimate_stage_resource_reservations
    from llm_sched.planning.schedule_reservations import (
...
def test_plan_single_core_schedule_allows_dma_at_layout_store_issue_with_vpu_prefix() -> None:
    from llm_sched.arch.capabilities import ArchitectureCapabilities
    from llm_sched.config.loader import load_target_profile
...
def test_plan_single_core_schedule_allows_dma_at_attention_mask_store_issue_with_vpu_prefix() -> None:
    from llm_sched.arch.capabilities import ArchitectureCapabilities
    from llm_sched.config.loader import load_target_profile
    from llm_sched.planning.schedule_duration import estimate_stage_resource_reservations
    from llm_sched.planning.schedule_reservations import (
...
def test_plan_single_core_schedule_allows_dma_at_rope_store_issue_with_vpu_prefix() -> None:
    from llm_sched.arch.capabilities import ArchitectureCapabilities
    from llm_sched.config.loader import load_target_profile
    from llm_sched.planning.schedule_duration import estimate_stage_resource_reservations
    from llm_sched.planning.schedule_reservations import (
...
def test_plan_single_core_schedule_allows_dma_at_embedding_store_issue_with_vpu_prefix() -> None:
    from llm_sched.arch.capabilities import ArchitectureCapabilities
    from llm_sched.config.loader import load_target_profile
    from llm_sched.planning.schedule_duration import estimate_stage_resource_reservations
    from llm_sched.planning.schedule_reservations import (
...
def test_plan_single_core_schedule_allows_dma_at_sdpa_store_issue_with_vpu_prefix() -> None:
    from llm_sched.arch.capabilities import ArchitectureCapabilities
    from llm_sched.config.loader import load_target_profile
    from llm_sched.planning.schedule_duration import estimate_stage_resource_reservations
    from llm_sched.planning.schedule_reservations import (
    ...
    capabilities = ArchitectureCapabilities.from_target_profile(target)
    sdpa_node = _make_sdpa_node("nig.node.attn.prefill", [1, 128, 1024])
    candidate = _make_tile_candidate("fixture.sdpa.store", "nig.node.attn.prefill", "SDPA", 48, 128, 128)
    reservations = estimate_stage_resource_reservations(
        macro_op="SDPA",
        stage="store",
        resource_set=["DMA"],
...
def test_plan_single_core_schedule_allows_dma_at_geglu_store_issue_with_vpu_prefix() -> None:
    from llm_sched.arch.capabilities import ArchitectureCapabilities
    from llm_sched.config.loader import load_target_profile
    from llm_sched.planning.schedule_duration import estimate_stage_resource_reservations
    from llm_sched.planning.schedule_reservations import (
...
def test_plan_single_core_schedule_allows_dma_during_kvload_vpu_tail() -> None:
    from llm_sched.arch.capabilities import ArchitectureCapabilities
    from llm_sched.config.loader import load_scenario_profile, load_target_profile
    from llm_sched.contracts.tiling_plan import TilingPlanArtifact
    from llm_sched.planning.memory_planner import plan_memory_artifact
    ...
    blocks = {block.block_id: block for block in schedule.blocks}
    kvload_0 = blocks["sched.nig.node.kvload.0.dma_in"]
    rope_table_dma = blocks["sched.nig.node.rope_table.dma_in"]
...
def test_plan_single_core_schedule_allows_dma_during_embedding_lookup_vpu_tail() -> None:
    from llm_sched.arch.capabilities import ArchitectureCapabilities
    from llm_sched.config.loader import load_scenario_profile, load_target_profile
    from llm_sched.contracts.tiling_plan import TilingPlanArtifact
    from llm_sched.planning.memory_planner import plan_memory_artifact
    ...
    blocks = {block.block_id: block for block in schedule.blocks}
    embedding_dma = blocks["sched.nig.node.embedding.dma_in"]
    elem_add_dma = blocks["sched.nig.node.elem.add.dma_in"]
    ...
    assert elem_add_dma.issue_slot == embedding_dma.issue_slot + dma_window[2]
    assert elem_add_dma.issue_slot < embedding_dma.issue_slot + embedding_dma.duration_slots


def test_plan_single_core_schedule_allows_dma_during_rope_table_vpu_tail() -> None:
    from llm_sched.arch.capabilities import ArchitectureCapabilities
    from llm_sched.config.loader import load_scenario_profile, load_target_profile
    from llm_sched.contracts.tiling_plan import TilingPlanArtifact
    from llm_sched.planning.memory_planner import plan_memory_artifact
    ...
    blocks = {block.block_id: block for block in schedule.blocks}
    rope_table_dma = blocks["sched.nig.node.rope.table.dma_in"]
    elem_add_dma = blocks["sched.nig.node.elem.add.dma_in"]
...
def test_plan_single_core_schedule_delays_vpu_after_heavier_attention_mask_prep() -> None:
    from llm_sched.config.loader import load_scenario_profile, load_target_profile
    from llm_sched.contracts.tiling_plan import TilingPlanArtifact
    from llm_sched.planning.memory_planner import plan_memory_artifact
    from llm_sched.planning.single_core_scheduler import plan_single_core_schedule
    ...
    def _build_schedule(original_op_kind: str) -> object:
        ...
        return plan_single_core_schedule(bound_nig, memory_plan, tiling_plan, target, scenario)
    ...
    trilu_mask_compute = trilu_blocks["sched.nig.node.mask.compute"]
    add_mask_compute = add_blocks["sched.nig.node.mask.compute"]
    trilu_elem_compute = trilu_blocks["sched.nig.node.elem.add.compute"]
    add_elem_compute = add_blocks["sched.nig.node.elem.add.compute"]
...
def test_plan_single_core_schedule_characterizes_layout_fallback_dma_overlap() -> None:
    from llm_sched.config.loader import load_scenario_profile, load_target_profile
    from llm_sched.contracts.tiling_plan import TilingPlanArtifact
    from llm_sched.planning.memory_planner import plan_memory_artifact
    from llm_sched.planning.single_core_scheduler import plan_single_core_schedule
    ...
    blocks = {block.block_id: block for block in schedule.blocks}
    layout_dma = blocks["sched.nig.node.layout.dma_in"]
    layout_prepare = blocks["sched.nig.node.layout.prepare"]
    layout_compute = blocks["sched.nig.node.layout.compute"]
    layout_store = blocks["sched.nig.node.layout.store"]
    elem_dma = blocks["sched.nig.node.elem.add.dma_in"]
