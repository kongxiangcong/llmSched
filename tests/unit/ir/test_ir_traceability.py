...
def test_ir_layers_preserve_traceability_fields() -> None:
    ...
    assert graph.nodes[0].source_ref == ["onnx::MatMul_0"]
    assert nig.nodes[0].shape == [1, 128, 1152]
    assert nig.nodes[0].attrs == {"weight_dtype": "int4", "group_size": 128}
    assert nig.nodes[0].audit_ref.graph_node_ids == ["graph.node.0"]
    assert schedule.blocks[0].audit_ref.nig_node_ids == ["nig.node.0"]
    assert descriptor.descriptors[0].audit_ref.schedule_block_ids == ["sched.block.0"]
