...
def test_run_sweep_analysis_writes_delta_report(tmp_path: Path) -> None:
    ...
    assert report.comparisons[0].prefill_compare.projection_read_bytes_weight.baseline_value >= 0.0
    assert report.comparisons[0].prefill_compare.attention_read_bytes_kv_cache.candidate_value >= 0.0
    assert report.comparisons[0].prefill_compare.other_write_bytes_activation.candidate_value >= 0.0
