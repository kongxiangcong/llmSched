...
def test_run_sweep_analysis_writes_delta_report(tmp_path: Path) -> None:
    ...
    assert "fitted_work_cycles" in report.run_records[0].metrics
    assert "tokens_per_fitted_work_cycle" in report.run_records[0].metrics
    assert "projection_fitted_work_cycles" in report.run_records[0].metrics
    ...
    prefill_compare = comparison.prefill_compare
    assert prefill_compare is not None
    baseline_run = next(
        run for run in report.run_records if run.target_profile_name == report.baseline_target_profile_name
    )
    candidate_run = next(
        run for run in report.run_records if run.target_profile_name == comparison.candidate_target_profile_name
    )
    metric_delta = next(delta for delta in comparison.metric_deltas if delta.metric_name == "estimated_cycles")
    metric_names = {delta.metric_name for delta in comparison.metric_deltas}
    assert {
        "projection_schedule_compression_cycles",
        "projection_schedule_compression_ratio",
        "projection_schedule_overhang_cycles",
    ...
    assert prefill_compare.estimated_cycles.delta_value == metric_delta.delta_value
    assert baseline_run.metrics["fitted_work_cycles"] >= baseline_run.metrics["estimated_cycles"]
    assert candidate_run.metrics["fitted_work_cycles"] >= candidate_run.metrics["estimated_cycles"]
    assert prefill_compare.fitted_work_cycles.baseline_value == baseline_run.metrics["fitted_work_cycles"]
    assert prefill_compare.fitted_work_cycles.candidate_value == candidate_run.metrics["fitted_work_cycles"]
    assert prefill_compare.critical_path_cycles.baseline_value > 0.0
    assert prefill_compare.critical_path_cycles.candidate_value > 0.0
    assert prefill_compare.tokens_per_fitted_work_cycle.baseline_value > 0.0
    assert prefill_compare.tokens_per_fitted_work_cycle.candidate_value > 0.0
    assert prefill_compare.fitted_cycles_per_token.baseline_value > 0.0
    assert prefill_compare.fitted_cycles_per_token.candidate_value > 0.0
    assert prefill_compare.projection_fitted_work_cycles.baseline_value >= prefill_compare.projection_cycles.baseline_value
    assert prefill_compare.projection_fitted_work_cycles.candidate_value >= prefill_compare.projection_cycles.candidate_value
    assert prefill_compare.projection_cycles.baseline_value > 0.0
    assert prefill_compare.projection_cycles.candidate_value > 0.0
    assert prefill_compare.projection_bytes.baseline_value > 0.0
    assert prefill_compare.attention_bytes.candidate_value > 0.0
    assert prefill_compare.projection_byte_share.baseline_value >= 0.0
    assert prefill_compare.attention_byte_share.candidate_value >= 0.0
    assert prefill_compare.projection_bytes_per_cycle.baseline_value > 0.0
    assert prefill_compare.attention_bytes_per_cycle.candidate_value > 0.0
    assert prefill_compare.projection_cycle_share.baseline_value > 0.0
    assert prefill_compare.attention_cycle_share.candidate_value > 0.0
    assert prefill_compare.projection_schedule_compression_cycles.baseline_value >= 0.0
    assert prefill_compare.attention_schedule_compression_ratio.candidate_value >= 0.0
    assert prefill_compare.other_schedule_overhang_cycles.candidate_value >= 0.0
    assert prefill_compare.projection_read_bytes_ddr.baseline_value >= 0.0
    assert prefill_compare.attention_read_bytes_vmem.candidate_value >= 0.0
    assert prefill_compare.projection_read_bytes_ddr_backed_staged.baseline_value >= 0.0
    assert prefill_compare.projection_read_bytes_ddr_persistent.delta_value == 0.0
    assert prefill_compare.other_write_bytes_vmem_local.candidate_value >= 0.0
    assert prefill_compare.projection_read_bytes_weight.baseline_value >= 0.0
    assert prefill_compare.attention_read_bytes_kv_cache.candidate_value >= 0.0
    assert prefill_compare.other_write_bytes_activation.candidate_value >= 0.0
    assert prefill_compare.projection_compute_cycles.baseline_value >= 0.0
    assert prefill_compare.attention_memory_cycles.candidate_value >= 0.0
    assert prefill_compare.sync_sync_cycles.candidate_value >= 0.0
    assert prefill_compare.projection_occupied_slots.baseline_value > 0.0
    assert prefill_compare.attention_occupied_slots.candidate_value > 0.0
    assert prefill_compare.projection_occupied_slots_per_token.baseline_value > 0.0
    assert prefill_compare.other_occupied_slots_per_token.candidate_value >= 0.0
    prefill_compare_payload = prefill_compare.model_dump(mode="json")
    assert {
        "projection_schedule_compression_cycles",
        "projection_schedule_compression_ratio",
        "projection_schedule_overhang_cycles",
    ...
    assert comparison.layer_deltas[0].baseline_cycle_share > 0.0
    assert comparison.layer_deltas[0].change_direction in {"up", "down", "flat"}
