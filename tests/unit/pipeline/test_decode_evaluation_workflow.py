...
def test_run_decode_evaluation_writes_report_and_updates_manifest(
    ...
    assert report.token_latency.phase_attribution["kv_io"].schedule_compression_cycles >= 0.0
    assert report.token_latency.phase_attribution["kv_io"].schedule_compression_ratio >= 0.0
    ...
    assert list(report.token_latency.phase_attribution["projection"].per_core_occupied_slots) == ["0", "1"]
    assert list(report.token_latency.phase_attribution["projection"].per_core_span_slots) == ["0", "1"]
